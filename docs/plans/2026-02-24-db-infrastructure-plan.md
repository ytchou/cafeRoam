...
console.log(
  `Generated seed migration with ${tags.length} tags → ${outputPath}`
);
```
...
describe('JobQueue', () => {
  ...
  describe('claim', () => {
    ...
    it('returns a job when one is pending', async () => {
      ...
      expect(mockRpc).toHaveBeenCalledWith('claim_job', {
...
describe('handleEnrichShop', () => {
  ...
  beforeEach(() => {
    ...
    mockEnrichShop.mockResolvedValue({
      ...
      modeScores: { work: 0.85, rest: 0.5, social: 0.4 },
    });
...
export async function handleEnrichShop(
  ...
  const { data: tags } = await supabase.from('taxonomy_tags').select('id');
        ...
        data: [
          {
            ...
            taxonomy_tags: { label: 'WiFi Available', label_zh: '提供 WiFi' },
          },
        ],
        error: null,
      }),
...
export async function handleGenerateEmbedding(job: QueuedJob): Promise<void> {
  ...
  const tagLabels = (shopTags ?? [])
    .map(
      (st: any) => `${st.taxonomy_tags?.label} (${st.taxonomy_tags?.label_zh})`
    )
    .join(', ');
...
describe('handleStalenessSweep', () => {
  ...
  it('queues enrich_shop jobs for stale shops', async () => {
    ...
    expect(mockEnqueue).toHaveBeenCalledWith('enrich_shop', {
      shop_id: 'shop-1',
    });
    expect(mockEnqueue).toHaveBeenCalledWith('enrich_shop', {
      shop_id: 'shop-2',
...
describe('handleWeeklyEmail', () => {
  ...
  beforeEach(() => {
    ...
    mockNot.mockResolvedValue({
      data: [{ id: 'user-1', profiles: { display_name: 'Alice' } }],
      error: null,
    });
...
const scheduler = new WorkerScheduler((jobType, payload, options) =>
  queue.enqueue(jobType, payload, options)
);
