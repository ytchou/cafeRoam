...
<header className="mb-4 flex items-center justify-between">
  <h1 className="text-[28px] font-bold text-[#1A1918]" style={BRICOLAGE_STYLE}>
    探索
  </h1>
...
```tsx
<div className="mb-3 flex items-center justify-between">
  <h2 className="text-lg font-bold text-[#1A1918]" style={BRICOLAGE_STYLE_SM}>
    Browse by Vibe
  </h2>
  <Link
    href="/explore/vibes"
...
<main className="min-h-screen bg-[#FAF7F4] px-5 pt-6 pb-24 lg:px-8">
  ...
  <div className="lg:flex lg:gap-8">
    ...
    <div
      className={
        isDesktop ? 'w-[400px] shrink-0 rounded-2xl bg-[#F5F4F1] p-6' : ''
      }
    >
...
}
{
  isDesktop ? (
    ...
    <ul className="flex flex-col gap-3">{/* existing mobile shop rows */}</ul>
  );
}
```
...
```tsx
<div
  className={`flex flex-col gap-4 px-5 pt-2 pb-24 lg:px-8 ${isDesktop ? 'grid grid-cols-2' : ''}`}
>
  {notes.map((note) => (
    <CommunityCardFull
      key={note.checkinId}
