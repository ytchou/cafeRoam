...
**Root cause:** Classic IDF implementation mistake. Standard IDF formula requires `df(tag)` = number of _documents_ containing the term. The code counted total occurrences instead, which works only if the input is guaranteed to have unique tag IDs per document.
