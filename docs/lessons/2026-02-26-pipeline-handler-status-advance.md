...
**Root cause:** Each handler was written to focus on its own task (run LLM, store tags, etc.) and enqueue the next job — the status advance for the *next* stage felt like "the next handler's job", so it fell through the gap.
**Prevention:** Each handler that enqueues a follow-up job owns two responsibilities: (1) do its own work, (2) advance `processing_status` to the *next* stage name before enqueuing. The status update and the `queue.enqueue` call should appear together. Write an assertion in the handler's test that the DB update includes the expected `processing_status` value.
