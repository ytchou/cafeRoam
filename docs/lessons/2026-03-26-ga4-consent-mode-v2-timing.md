...
**Root cause:**
GA4 consent mode v2 requires the `gtag('consent', 'default', ...)` call to arrive in the `dataLayer` queue *before* the GA4 script processes it. React `useEffect` runs after paint, at the same time as deferred scripts — there is no guaranteed ordering.
