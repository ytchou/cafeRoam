...
Supabase's JWT claim hook (`custom_access_token_hook`) runs at **token mint time**, not at read time. `exchangeCodeForSession()` mints the token _before_ any subsequent DB writes. Those writes are not reflected in the current token until the token is refreshed (either by expiry + auto-refresh, or by explicit `refreshSession()` call).
