...
**Issues fixed:**
- [Important] classify_shop_photos.py — Removed per-row DB writes; added `_get_existing_category_counts()` for global cap enforcement; `_enforce_cap()` now accepts `remaining_slots`; `_batch_write()` issues one `update().in_()` per final category (I-1, I-2)
...
- [Important] test_classify_shop_photos.py — `test_menu_cap_enforcement` rewrites to assert `len(update_calls)==2` and in_() arg lengths; new `test_menu_cap_enforcement_respects_globally_classified_photos` test (I-5, I-2 test coverage)
