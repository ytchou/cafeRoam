...
- **Semantic search is auth-gated:** The chatbox on the landing page is visible to all users but prompts login when submitted without an active session. Free (auth) users get 5 AI searches per day; after the cap, results fall back to keyword search with an upgrade nudge. Member tier gets unlimited AI searches.
