...
- **Community feed access:** The community feed (`/explore/community`) shows all public check-ins with review text. Requires authentication — anonymous users cannot view the feed. Feed supports district and vibe tag filters.
...
- **Role hierarchy:** Five levels — `user` (anonymous, no auth), `auth-user` (signed up), `blogger`, `partner`, `admin`. Community feed readable by `auth-user` and above. Detailed role permissions deferred to a separate design session.
