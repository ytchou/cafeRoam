...
| Explore               | `/explore/vibes/[slug]` — Vibe category page with collapsible map panel (expanded by default), district filter chips, and "Near Me" toggle. Shows all matching shops by default (no geo-gating). Map uses existing Mapbox integration with bidirectional pin-list sync. | 2     |
