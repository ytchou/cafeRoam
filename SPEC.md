...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----- |
| Data pipeline         | One-time data collection (Cafe Nomad seed → Apify/Google Maps verify + scrape) + ongoing enrichment (Claude Haiku + embedding generation) + daily batch cron (03:10 Asia/Taipei) that processes all pending shop submissions                                                         | 1     |
...
| User profile          | Private profile page with 3 tabs: stamps, lists preview (with a "View all lists →" CTA), and check-in history. On mobile, bottom nav is 4 tabs: Home, Find, Explore, Profile.                                                                                                        | 2     |
...
| Claude Sonnet 4.6 (Anthropic) + OpenAI GPT-5.4-mini/nano | LLM enrichment — `enrich_shop` on Claude Sonnet 4.6 (`AnthropicLLMAdapter`); `extract_menu_data`, `classify_photo`, `summarize_reviews`, `assign_tarot` on OpenAI GPT-5.4-mini/nano (`OpenAILLMAdapter`). Routed by `HybridLLMAdapter`. | Yes — `LLMProvider` protocol unchanged; rollback via `LLM_PROVIDER=anthropic` single env var flip | Enrichment quality degraded; fallback to manual |
...
**LLM enrichment rationale:** Hybrid routing introduced in DEV-304. `enrich_shop` (taxonomy tagging) uses Claude Sonnet 4.6 via `AnthropicLLMAdapter` — Claude outperforms on instruction-following when output is constrained to a predefined taxonomy list, critical for tag quality. The 4 remaining methods (`extract_menu_data`, `classify_photo`, `summarize_reviews`, `assign_tarot`) use OpenAI GPT-5.4-mini/nano via `OpenAILLMAdapter`, which are significantly cheaper for vision and summarization tasks. All routing is handled by `HybridLLMAdapter`, which implements the same `LLMProvider` protocol — no callers changed. Rollback: set `LLM_PROVIDER=anthropic` to route all methods through Anthropic. User maintains separate Anthropic and OpenAI service accounts for CafeRoam.
...
- **Community shop submissions:** Any authenticated user can submit a Google Maps URL to add a café. Submissions are queued and processed in a daily batch at 03:10 Asia/Taipei (scrape → enrich → embed). Processing typically completes within 24 hours of submission. Submissions are held in `pending_review` for admin approval before going live. Rate-limited to 5 submissions per day per user. Users can track their submission status on `/submit`. Admin reviews on the `/admin` page with approve/reject + canned rejection reasons. PDPA cascade on account deletion must include `shop_submissions` rows (already handled via FK ON DELETE CASCADE).
