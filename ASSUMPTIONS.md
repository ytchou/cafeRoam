...
| 2   | Enriched data (menus, offerings, vibes) can be obtained at scale for Taipei shops | Medium     | Google Maps reviews contain this info; Apify scraper tested. 3-pass pipeline designed: seed → verify → scrape.                                                                                                                       | Medium           | FATAL — no enriched data = no semantic search                | Pre-build week 0: run pipeline on 30 shops, measure time per shop                 |
...
| 7   | Cafe Nomad's open API data is reliable enough for bootstrapping                   | High       | Multiple apps use it. Spot-checked — reliable for names/coordinates, but 75% missing hours, 29% zero-rating shells, 15+ closed shops. Used as seed list only (names + lat/lng). Google Maps is source of truth for operational data. | Low              | Minor — alternative data sources exist                       | Day 1 of pre-build: spot-check 20 shops                                           |
...
| T7  | Shop photo URLs from Google Maps CDN are stable across re-scrapes (no URL rotation) | Medium     | Google CDN URL structure has been stable in practice; same content = same URL path in observations | Medium           | Low — if URLs rotate, old classified rows become orphaned (wrong category on a dead URL). Current persistence uses upsert on `(shop_id, url)`, so rotated URLs insert new rows with NULL category and get re-classified correctly. The old row stays with its previous category but points to a 404. Fix: full photo refresh on re-scrape, or periodic orphan cleanup job. | Phase 2: monitor shop_photos for rows with old upload dates that no longer appear in fresh scrapes |
