...
def _make_chainable(final_data: list) -> MagicMock:
    """Return a MagicMock that chains .select().eq().is_().in_() etc. and returns final_data on .execute()."""
    result = MagicMock()
    result.execute.return_value = MagicMock(data=final_data)
    for method in ("select", "eq", "is_", "in_", "neq", "filter", "order", "limit"):
...
def _make_db(
    ...
    db = MagicMock()
    shops_chain = _make_chainable(shops_data)
    jobs_chain = _make_chainable(pending_jobs or [])
    table_mocks: dict[str, MagicMock] = {"shops": shops_chain, "job_queue": jobs_chain}
...
class TestBackfillCommunitySummaries:
    ...
    async def test_skips_shops_with_existing_pending_or_claimed_jobs(self):
        """Script does not enqueue duplicate jobs when a SUMMARIZE_REVIEWS job is already pending or claimed."""
        db = _make_db(
            shops_data=[
                {"id": "shop-taipei-01", "name": "虎記商行"},
                {"id": "shop-taipei-02", "name": "木子鳥"},
